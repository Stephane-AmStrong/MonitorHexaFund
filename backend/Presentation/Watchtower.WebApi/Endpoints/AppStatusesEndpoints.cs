...
public static class AppStatusesEndpoints
{
    ...
    private static  async Task<IResult> GetByQueryParameters(IQueryHandler<GetAppStatusQuery, PagedList<AppStatusResponse>> handler, [AsParameters] AppStatusQueryParameters queryParameters, HttpResponse response, IOptions<JsonOptions> jsonOptions, CancellationToken cancellationToken)
    {
        var appStatusesResponse = await handler.HandleAsync(new GetAppStatusQuery(queryParameters), cancellationToken);

        response.Headers.Append("X-Pagination", JsonSerializer.Serialize(appStatusesResponse.MetaData, jsonOptions.Value.SerializerOptions));
